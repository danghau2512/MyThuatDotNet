    ...
    public class SearchController : ControllerBase
    {
            ...
            [FromQuery] int take = 8)
        {
            ...
            var data = await _context.Products
                .AsNoTracking()
                .Where(p => p.IsActive && p.Name.Contains(keyword)) // ✅ filter active
                .OrderBy(p => p.Name)
                .Select(p => new ProductSuggestDto
                {
                    Id = p.Id,
                    Name = p.Name,
                    Price = p.Price * (100 - (p.DiscountDefault ?? 0)) / 100m, // ✅ final price
            ...
            [FromQuery] int pageSize = 8)
        {
            ...
            IQueryable<MyThuatShop.Api.Models.Product> query = _context.Products
                .AsNoTracking()
